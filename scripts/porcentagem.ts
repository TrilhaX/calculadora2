export class Porcentagem {
    static calcularPorcentagem() {
        const valor = parseFloat((document.querySelector('#valorPorcento') as HTMLInputElement).value);
        const percentual = parseFloat((document.querySelector('#percentual') as HTMLInputElement).value);
